class WTFEquipKSG extends KSGShotgun;
...
{	
	if ( IsWTFSupportSpec && (bIsReloading || MagAmmoRemaining >= MagCapacity) ) {		
		if (MyFm.ProjectileClass == MyFm.Default.ProjectileClass)
		{
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(class'WTF.WTFMessages',0);
			MyFm.ProjPerFire=1;
			MyFm.ProjectileClass=Class'WTF.WTFEquipKSGSlug';
			MyFm.Spread *= 0.35;
			MyFm.AimError *= 0.35;
		}
		else
...
defaultproperties
{
	FireModeClass(0)=Class'WTF.WTFEquipKSGFire'
	Description="Secondary to toggle tight/wide spread. WTF Support: double-reload or reload while full to load Shot/Slugs"
	PickupClass=Class'WTF.WTFEquipKSGPickup'
}
